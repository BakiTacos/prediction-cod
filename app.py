...
st.write("Model ini memprediksi probabilitas pesanan COD mengalami **Pengiriman Gagal / Paket Hilang**.")
...
with st.form("cod_form"):
    ...
    with col1:
        ...
        same_city = st.selectbox("Kota sama (misal Tangerang)?", ["Tidak", "Ya"])
        same_province = st.selectbox("Provinsi sama (misal Banten)?", ["Tidak", "Ya"])
    ...
    with col2:
        ...
        jasa_kirim = st.text_input("Jasa Kirim (contoh: SPX)", "SPX")
...
if submitted:
    ...
    if label == 1:
        st.error("⚠ Pesanan ini berisiko **tinggi** gagal kirim.")
    else:
